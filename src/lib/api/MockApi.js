...
const CONTACTS = [
  {
    id: 2,
    baseUrl: "/default-avatar.jpg",
    names: [{ firstName: "Кот", lastName: "Ослов", description: "aaaa" }],
  },
  {
    id: 5,
    baseUrl: "/vlad.jpg",
    names: [{ firstName: "Владимир", lastName: "Путин", description: "aaaa" }],
  },
];
...
const CHATS = [
  {
    ...
    participants: { 123: Date.now(), 2: Date.now() },
    newMessages: 0,
  },
  {
    id: 5,
    ...
    status: 1,
    lastMessage: {
      text: "Трудолюбие — это вообще отдельный талант, это не просто, извините, резиновая попа",
      time: Date.now() - 1000 * 600,
    },
    participants: { 123: Date.now(), 5: Date.now() },
    newMessages: 0,
  },
];
...
export default class MockApi extends API {
  ...
  async sync() {
    ...
    chats.forEach((chat) => {
      ...
      if (!exists)
        ...
        if (exists.videoConversation)
          exists.videoConversation = videoConversation;
        if (exists.status) exists.status = status;
  ...
  async sendMessage(text, chatId, params) {
    const msg = {
      id:
        MESSAGES[chatId][MESSAGES[chatId].length - 1].id +
  ...
  async addContact(name, phone) {
    ...
    contacts[id] = {
      ...
      names: [
        { firstName, lastName: lastName || "", description: "I love Svelte" },
      ],
      added: true,
    };
