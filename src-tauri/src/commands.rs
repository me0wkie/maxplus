...
pub async fn init(
    ...
    if let Some(uid) = user_id { state.client.set_user_id(uid).await; }
    if let Some(t) = token { state.client.set_token(t).await; }

    state.client.connect(device_id, mt_instance, true).await
...
pub async fn sync_client(state: State<'_, AppState>) -> Result<Value, String> {
    ...
    if let Some(id) = r.payload.pointer("/profile/contact/id").and_then(|v| v.as_u64()) {
...
pub async fn fetch_history(
    ...
    let r = state.client.fetch_history(chat_id, from_time, 0, amount.unwrap_or(200)).await.map_err(|e| e.to_string())?;
    serde_json::to_value(r.payload).map_err(|e| e.to_string())
}
